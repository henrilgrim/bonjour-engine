...
export function usePauseControl() {
    ...
    const handleEndPause = useCallback(async () => {
        ...
        // Limpa notificações de pausa após finalizar
        const currentReason = store.correlationPause?.type_operation?.reason;
        notificationManager.clearPauseNotifications(currentReason);

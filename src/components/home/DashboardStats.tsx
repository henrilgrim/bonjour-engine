...
const StatsCard = memo(function StatsCard({
    ...
    return (
        <Card className="group shadow-none rounded-2xl bg-glass border border-glass-border hover:bg-surface-elevated/50 transition-all duration-300 hover:scale-[1.02] hover:shadow-glow">
            <CardContent
                className={[
...
export default function DashboardStats({
    ...
    const stats = useMemo(
        () => [
            ...
            {
                label: "Disponíveis",
                value: nf.format(available),
                icon: CheckCircle,
                color: "status-available",
            },
            {
                label: "Indisponíveis",
                value: nf.format(unavailable),
                icon: XCircle,
                color: "status-unavailable",
            },
            {
                label: "Em Uso",
                value: nf.format(busy),
                icon: PhoneCall,
                color: "status-busy",
            },
            {
                label: "Tocando",
                value: nf.format(ringing),
                icon: PhoneIncoming,
                color: "status-ringing",
            },
            {
                label: "Em Espera",
                value: nf.format(waiting),
                icon: Hourglass,
                color: "status-waiting",
            },
            {
                label: "Em Pausa",
                value: nf.format(paused),
                icon: Coffee,
                color: "status-paused",
            },
        ],

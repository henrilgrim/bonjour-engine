...
export default function AgentApproveReasonDialog({
    ...
    useEffect(() => {
        ...
        const loadHistory = async () => {
            ...
            try {
                // Só chama se agent.idAgentDb existir
                console.log("agent.idAgentDb", agent.idAgentDb);
                if (agent.idAgentDb) {
                    const list = await agentListRequest(agent.idAgentDb);
                    setHistory(list);
                } else {
                    console.warn(
                        "Agent ID não encontrado para carregar histórico:",
                        agent.login
                    );
                    setHistory([]);
                }
        ...
        loadHistory();
    }, [open, agent.login, agent.idAgentDb]); // Remove agentListRequest das dependências

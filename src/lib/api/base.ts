 ...
 * 
 * Este arquivo centraliza a configuração das APIs do sistema.
 * Inclui interceptors para tratamento de erros, autenticação e logging.
 */
 ...
 */
export function createApiInstance(baseURL: string, config?: AxiosRequestConfig): AxiosInstance {
  ...
  instance.interceptors.response.use(
    ...
    (error: AxiosError) => {
      ...
      if (error.response?.status === 401) {
        apiError.message = 'Acesso não autorizado. Faça login novamente.'
 ...
 */
export function createPrivateApiInstance(baseURL: string, token: string): AxiosInstance {
  return createApiInstance(baseURL, {
    headers: {

...
import { useFirebaseSupervisors, type SupervisorOnline } from "@/hooks/use-firebase-supervisors";
import { 
...
import type { CentralNotification, NotificationType } from "./types";

export function useNotifications() {
    const store = useNotificationStore();
    const { config, playNotificationSound, isEnabled } = useNotificationsConfig();
    ...
    const notify = useCallback(
        async (notification: Omit<CentralNotification, "id" | "timestamp">): Promise<string | undefined> => {
            // Verifica se o tipo de notificação está habilitado
            const typeEnabled = isEnabled(notification.type);
            if (!typeEnabled) {
                console.log(`Notificação do tipo ${notification.type} está desabilitada`);
                return undefined;
            }
            ...
            const soundType = notification.type === "system" 
                ? (notification.variant === "error" ? "rejected" : notification.variant === "success" ? "approved" : "system")
                : notification.type;
            
            playNotificationSound(soundType as any);

            // Envia PWA notification se página não estiver ativa
    ...
    const notifyMessage = useCallback(
        async (
            ...
            if (data?.messageId) {
                if (store.isMessageViewed(data.messageId) || store.isMessagePreviewed(data.messageId)) {
                    console.log("Mensagem já visualizada, pulando notificação:", data.messageId);
                    return undefined;
                }
            }
    ...
    const notifyPause = useCallback(
        async (
            ...
            return notify({
                ...
                icon: variant === "error" ? "cancel" : variant === "success" ? "check" : "pause",
                data,
            });
        },
        [notify]
    ...
    const notifySystem = useCallback(
        async (
            ...
            return notify({
                ...
                icon: variant === "error" ? "cancel" : variant === "success" ? "check" : "alert",
                data,
            });
        },
        [notify]
    ...
    const notifyIncoming = useCallback(
        (supervisor: SupervisorOnline, message: ChatMessage) => {
            ...
            const senderName = supervisorNameById.get(supervisor.id) ?? supervisor.name ?? "Supervisor";
    ...
    useEffect(() => {
        ...
        console.log("Configurando listener de pausas para:", { user: user.login, company: company.accountcode });
        ...
        const unsubscribe = listenPauseRequestStatus(
            ...
            (data: PauseRequestPayload | null) => {
                ...
                if (processedPauseResponsesRef.current.has(responseKey)) {
                    console.log("Resposta de pausa já processada, ignorando:", responseKey);
                    return;
                }

                processedPauseResponsesRef.current.add(responseKey);
                console.log("Processando resposta de pausa:", { status: data.status, reason: data.reasonName });

                // Centraliza toda a lógica de notificação de pausa aqui
                if (data.status === "approved") {
                    // Som + notificação in-app + PWA
                    notificationManager.playNotificationSound("approved", config);
                    notificationManager.sendPauseNotification("approved", data.reasonName);
                    ...
                    // Som + notificação in-app + PWA
                    notificationManager.playNotificationSound("rejected", config);
                    ...
                    notifyPause(
                        "Pausa rejeitada ❌",
                        data.rejectionReason || `Sua solicitação foi rejeitada: ${data.reasonName}`,
                        "error"
                    );
                }
                ...
                if (processedPauseResponsesRef.current.size > 20) {
                    const entries = Array.from(processedPauseResponsesRef.current);
                    processedPauseResponsesRef.current = new Set(entries.slice(-10));
                }
            }
        );
    ...
    const sendBreakExceededNotification = useCallback(
        (reasonName: string, timeExceeded: string) => {
            ...
            notificationManager.sendPauseNotification("exceeded", reasonName, timeExceeded);
    ...
    const sendBreakWarningNotification = useCallback(
        (reasonName: string, timeRemaining: string) => {
            ...
            notificationManager.sendPauseNotification("warning", reasonName, timeRemaining);
    ...
    useEffect(() => {
        const initPermissions = async () => {
            ...
            if (granted) {
                notifySystem("Notificações ativas", "Você receberá alertas de mensagens e pausas", "success");
            } else {
                notifySystem("Notificações desabilitadas", "Ative as notificações para receber alertas importantes", "error");
            }
        };
...
export function useSimpleNotifications() {
    const { notify, notifySystem, notifyMessage, notifyPause } = useNotifications();

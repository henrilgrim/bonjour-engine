...
export function getChatsCollection(accountcode: string) {
    return collection(getManagerPanelDoc(accountcode), "chats").withConverter(
        chatMetaConverter
    );
}
...
async function ensureChatExists(params: {
    accountcode: string;
    chatId: string;
    agentLogin: string;
}) {
...
export async function sendMessage(
    ...
    await addDoc(col, {
        ...
        senderLogin: isAgent
            ? agentLogin
            : (message as any).senderLogin ?? null,
        createdAt: serverTimestamp(),
        read: false,
    } as any);
...
export async function subscribeToAllMessagesForAgent_byEnumeratingChats(
    accountcode: string,
    agentLogin: string,
    onNext: (msgs: ChatMessage[]) => void,
    onError?: (e: unknown) => void
    ...
    try {
        // 1) Lista todos os chats e filtra pelo prefixo do agente + inclui chat global
        const snap = await getDocs(query(getChatsCollection(accountcode)));
        const prefix = `ag_${agentLogin}__sup_`;
        const chatIds = snap.docs
            .map((d) => d.id)
            .filter((id) => id.startsWith(prefix) || id === "global_chat");
...
export async function listChatsForAgent(
    accountcode: string,
    agentLogin: string
): Promise<ChatMeta[]> {
    ...
    const snap = await getDocs(qy);
    let retorno = snap.docs
        .map((d) => d.data())
        .filter((c) => c.id.startsWith(`ag_${agentLogin}__sup_`));
    retorno.sort((a, b) => {
        const ta =
...
export async function getUnreadMessagesForAgent(
    accountcode: string,
    agentLogin: string
): Promise<ChatMessage[]> {
    const qy = query(
        collectionGroup(firestore, "messages").withConverter(
 ...
 */
export async function getAgentSentMessages_byEnumeratingChats(
    accountcode: string,
    agentLogin: string
): Promise<ChatMessage[]> {
...
export function subscribeAllAccountMessagesStream(
    accountcode: string,
